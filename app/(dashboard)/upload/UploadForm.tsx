...
import { Upload, X, CheckCircle, AlertCircle, FileText, Loader2, Globe } from 'lucide-react'
...
export default function UploadForm({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function handleUpload() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (isAdmin === true && isShared) formData.append('isShared', 'true')
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex-1 px-4 md:px-6 py-6 space-y-6 max-w-2xl mx-auto w-full">
        {/* 管理者用: 全社共有ナレッジトグル（厳密に true のときのみ表示） */}
        {isAdmin === true && (
          <div className={cn(
            'rounded-2xl border p-4 transition-colors',
            isShared ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-white'
          )}>
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  ON にしてアップロードしたファイルは <strong>全ユーザーの見積もり・検索</strong>で参照されます。
                  単価表・歩掛・国交省資料など、全社で共有したい資料に使ってください。
                </p>
              </div>
            </label>
          </div>
        ...
        {pendingCount > 0 && (
          <button
            ...
            className={cn(
              'w-full font-semibold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 text-white',
              isAdmin === true && isShared
          ...
          >
            {isUploading ? (
              ...
              <>
                ...
                {pendingCount}件を{isAdmin === true && isShared ? '全社共有として' : ''}アップロード
              </>
            )}
          </button>
        )}
